    ...
    for idx, student_data in enumerate(st.session_state.generated_data):
        ...
        with st.container():
            ...
            with st.expander("👁️ Preview Grade Summary"):
                ...
                st.markdown(
                    f"""<div style="background-color: #f9f9f9; padding: 15px; border-radius: 5px; border: 1px solid #ddd; font-family: Arial, sans-serif;">
                    {student_data["Grade Summary"].replace(chr(10), '<br>')}
                    </div>""",
                    unsafe_allow_html=True
                )
                st.info("💡 Select and copy the text above (with Ctrl+C or Cmd+C) and paste into HubSpot's rich text editor.")
